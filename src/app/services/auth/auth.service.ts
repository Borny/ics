...
export class AuthService {
  ...
  private _userLoginFailed$: Subject<{ failed: boolean; message: string }> =
    new Subject<{ failed: boolean; message: string }>();
  ...
  private readonly GET_USER_URL = environment.apiUrl + '/user';
  private readonly GET_USERS = environment.apiUrl + '/get-users';
  private readonly ADMIN_LOGIN_URL = environment.apiUrl + '/admin-login';
  private readonly REQUEST_PASSWORD_URL =
    environment.apiUrl + '/request-reset-password';
  private readonly VALIDATE_PASSWORD_URL =
    environment.apiUrl + '/valid-password-token';
  ...
  private readonly DELETE_USER_URL = environment.apiUrl + '/user';
  ...
  public getUsers(): Observable<{ users: User[] }> {
    return this.http.get<{ users: User[] }>(`${this.GET_USERS}`);
  }
  ...
  public updateUser(userData): Observable<{ message: string }> {
    return this.http
      .put<{ message: string }>(
        `${this.UPDATE_USER_URL}/:${userData._id}`,
        userData
      )
      .pipe(shareReplay());
  }
  ...
  public adminLogin(adminLoginFormValue: AdminAuth): void {
    this.http
      .post<{ token: string; admin: string; expiresIn: number }>(
  ...
  public validPasswordToken(formValue: {
    resetToken: string;
  }): Observable<any> {
    // change the type// change the type
